...
func (h *handler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	// future
	futureFunc := future.FutureFunc[[]reflect.Value](h.method.Call, args)
	// The default execution time for the method is 10 seconds, and if it exceeds this limit, it will return.
	if out, err = futureFunc.GetWithTimeout(time.Second * 10); err != nil {
		err2 := httputil.NewError(500, err.Error())
		if err.Error() == "timed out" {
			err = errors.Info(err2, "ServeHTTP  request failed").Detail(err)
			repl.Error(w, err)
			return
		}
		repl.Error(w, err2)
...
func (p HandlerCreator) New(rcvr reflect.Value, method reflect.Method) (http.Handler, error) {
	...
	if narg == 2 {
		...
		if reqType.Kind() == reflect.Ptr {
			...
			xlog.Debug("method", method.Name, "arg type not a pointer:", reqType)
			return nil, syscall.EINVAL
		}
	}
	...
	if hasRet >= 0 {
		if errType := mtype.Out(hasRet); errType != typeOfError {
			xlog.Debug("method", method.Name, "returns", errType.String(), "not error")
			return nil, syscall.EINVAL
		}
	}
